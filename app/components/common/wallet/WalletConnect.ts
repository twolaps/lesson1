...
export const connectWallet = async (
	...
	try {
		const targetProvider: WalletProvider = walletProviders.get(walletType)?.provider as WalletProvider;
		if (targetProvider) {
			const accounts = await targetProvider.request({
				method: "eth_requestAccounts" 
			});

			console.log(`✅ ${walletType} 已连接成功！`);
		...
		console.error(`❌ 连接 ${walletType} 失败:`, error);
	}
}
