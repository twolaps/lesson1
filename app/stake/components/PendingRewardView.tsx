import { MetaNodeContext } from "@/app/components/common/wallet/context/MetaNodeContext";
...
export default function PendingRewardView({ stakeAddress, stakeAbi, myAddress }: PendingRewardViewProps) {
  ...
  const { isSuccess, isError, error } = useWaitForTransactionReceipt({
	...
	const {metaNodeBalance, refetchMetaNodeBalance} = useContext(MetaNodeContext);
  ...
  }, [isSuccess, isError, error, refetchPendingReward, refetchMetaNodeBalance]);
  ...
  return (
    <>
      ...
      <Typography sx={{mt: 4}} variant="h5" align="center">Your Balance: {bigintToString(metaNodeBalance, 4)} {symbolStr}</Typography>
